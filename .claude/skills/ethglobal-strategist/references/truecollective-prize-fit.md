...
| **Privy control を最低1つ**（policies / signers / key quorums / intents）| **MCP 決済ウォレット = Privy server wallet**（生鍵は gateway に無く、全署名が Privy RPC）＋ **per-session 支出上限**（`MCP_SESSION_SPEND_CAP_TINYBAR`、実装は gateway 側 `mcp/session.ts`。Hedera tx digest の raw-hash 署名には Privy policy が評価できる tx 構造が無いため。T092 / FR-028 / SC-011）。**加えて specs/mcp-auth-remediation-plan.md の OAuth 実装で per-principal 委任ウォレットに signer quorum + authorization key（`PRIVY_SIGNER_QUORUM_ID` / `PRIVY_AUTHORIZATION_PRIVATE_KEY`、`mcp/privyClient.ts`）を必須化済み**| **判断は解消済み（2026-09-10）**: authorization key を付ける案（旧「要判断」の (b)）を採用・実装済み。残作業は Hedera 実ホスト・HashScan verify・Privy 実ログインと同じ「ライブ未」枠＝実デプロイ・実 Privy App credentials を用意した上での authorization signature 欠如時の Privy 側拒否のライブ実測 ＝ `docs/submission/prize-requirements.md` §3 |
