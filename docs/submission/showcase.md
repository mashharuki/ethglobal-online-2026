...
A transfer-coupled rights runtime: when the NFT moves, access, licenses and future revenue move with it.

## Short description (≤ 280 chars)

NFT ownership and the *actual* right to use the content drift apart the moment a token is resold. TrueCollective keeps them in sync on Hedera: every transfer atomically revokes the old owner, grants the new one, preserves paid licenses that should survive, and redirects future revenue.
...
- **Payments**: x402 (`hedera:testnet` / `exact` / native asset) via the Blocky402 facilitator;
  ...
  resumable. Web buyers sign the Hedera transfer with their Privy embedded wallet; the MCP agent
...
- **Hedera — AI & Agentic Payments on Hedera.** The x402-gated Access Gateway is hosted against
  Hedera Testnet with the Blocky402 facilitator; the MCP agent completes a real paid request
  end-to-end in native HBAR. `RightsNFT` / `RightsRegistry` are verified on HashScan:
  <RightsNFT address> / <RightsRegistry address>. Setup, architecture and the payment flow are in
  the README.
- **Privy — Best Financial Flow.** Owners and buyers log in with the Privy embedded wallet; the
  complete flow (x402 payment in HBAR → on-chain receipt → revenue split creator/owner → decrypt)
  runs with Privy at the centre of the transfer, not as a decorative login.
- **Privy — Best B2B Financial Product.** The MCP payment wallet is a Privy server wallet: the
  gateway never holds the key, every signature is a Privy RPC, and each MCP session is bound to
  the receipts it bought with a hard per-session spend cap. The B2B use case is a data provider
  selling licensed datasets to autonomous agents with auditable, capped spending.
...
documents under `specs/` and `.specify/` were written 2026-09-02 → 03 with Spec Kit, as allowed by
the AI policy. Details: README → "Prior work and disclosure".
