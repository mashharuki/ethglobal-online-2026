...
| **Privy control を最低 1 つ**（policies / signers / key quorums / intents） | 🟡 | **設計判断（この行の旧 ❗）は解消済み**: specs/mcp-auth-remediation-plan.md の OAuth 実装で、signer quorum + authorization key を要件の「signers / key quorums」に対応する control として採用した。`mcp/privyClient.ts`（`createPrivyDelegationClient`）が、per-principal 委任ウォレットへの全ての署名/更新 RPC（`signSecp256k1Delegated` / `signTypedDataDelegated` / additional-signer attach）に `authorization_context: { authorization_private_keys: [PRIVY_AUTHORIZATION_PRIVATE_KEY] }` を付与する実装をコード上完了している。この鍵はウォレットの signer quorum（`PRIVY_SIGNER_QUORUM_ID`）のメンバーとして事前登録された P-256 鍵で、raw-hash 署名（`secp256k1_sign`）でも成立する認可制御＝**「誰が署名を要求できるか」を縛る**もの（署名対象の tx 内容・金額を Privy 側で検査する制御ではない。**支出額の強制は依然 gateway 側**＝per-session cap `MCP_SESSION_SPEND_CAP_TINYBAR` に加え、今回の remediation で `agent_grant` の予算envelope・`agent_principal_spend` の日次上限も追加、`apps/gateway/CONFIG.md` 参照）。**🟡 に留めている理由**（実装済みと Privy 側の実挙動を混同しない）：`test/node/privyClient.test.ts` はゲートウェイのクライアントが正しい `authorization_context` を**送っていること**は検証済みだが、これは我々のクライアント実装の確認であって、authorization signature を**欠いた/不正な**署名 RPC を実際の Privy API に投げたときに**Privy 側が実際に拒否する**ことはまだライブ実測していない（他のライブ切替手順と同じく実デプロイ・実 Privy App 待ち）。動画で見せる場合はこの拒否デモを優先する |
...
- [ ] Privy B2B: §3 の Privy control は signer quorum + authorization key で実装済み。上限超過の拒否、および authorization signature 欠如時に Privy が署名を拒否する場面を動画に（後者はライブ実測が残作業）
...
```
The embedded wallet and the server wallet APIs were easy to wire (eth_signTypedData_v4 for EIP-712 challenges, secp256k1_sign for the Hedera transaction digest). The gap we hit: for a raw 32-byte digest, a wallet policy has no amount, recipient or method to evaluate, so on a chain whose native transfers are not EVM transactions (Hedera) our per-session spend limit had to live in the gateway rather than in a Privy policy. We did end up building the other half of what we originally asked for on our side - an authorization-key requirement scoped per delegated wallet: our gateway client now attaches an authorization_context, signed with a P-256 key registered on that wallet's signer quorum, to every signSecp256k1/signTypedData call for a principal's own wallet. That constrains *who* on our side can request a signature (a call built without the registered authorization key is malformed by our own client's design), not *what* gets signed - the amount still isn't something a raw 32-byte digest carries for Privy to inspect, so spend limits still live entirely in our gateway. What is still missing is a native signing *budget* on raw-hash signing itself (a per-wallet or per-quorum-member spend cap enforced by Privy). We have not yet run this against a live Privy App with real credentials, so we can't yet confirm from Privy's side that it actually rejects a raw-hash signing request that omits or misauthorizes authorization_context - only that our own client always sends one. <add findings from that live test once the OAuth cutover is live>
```
