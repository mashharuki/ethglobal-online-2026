...
| **Privy control を最低 1 つ**（policies / signers / key quorums / intents） | 🟡 | **判断（この行の旧 ❗）は解消済み**: specs/mcp-auth-remediation-plan.md の OAuth 実装で、下の (b) 案（signer quorum + authorization key）をそのまま採用した。`mcp/privyClient.ts`（`createPrivyDelegationClient`）が、per-principal 委任ウォレットへの全ての署名/更新 RPC（`signSecp256k1Delegated` / `signTypedDataDelegated` / additional-signer attach）に `authorization_context: { authorization_private_keys: [PRIVY_AUTHORIZATION_PRIVATE_KEY] }` を付与する。この鍵はウォレットの signer quorum（`PRIVY_SIGNER_QUORUM_ID`）のメンバーとして事前登録された P-256 鍵で、raw-hash 署名（`secp256k1_sign`）でも成立する**認可制御**＝要件の「signers / key quorums」に該当する Privy control が1つ設定されている。**注意点は変わらず有効**: ① これは「誰が署名を要求できるか」を Privy 側で縛る制御であって、**支出額の強制は依然 gateway 側**（per-session cap `MCP_SESSION_SPEND_CAP_TINYBAR` に加え、今回の remediation で `agent_grant` の予算envelope・`agent_principal_spend` の日次上限も追加、`apps/gateway/CONFIG.md` 参照）。② 🟡 に留めている理由：`test/node/privyClient.test.ts` はゲートウェイが正しい `authorization_context` を**送っていること**は検証済みだが、authorization signature を**欠いた/不正な**署名 RPC を実際の Privy API に投げて**Privy 側が拒否する**場面はまだライブ実測していない（他のライブ切替手順と同じく実デプロイ・実 Privy App 待ち）。動画で見せる場合はこの拒否デモを優先する |
...
```
The embedded wallet and the server wallet APIs were easy to wire (eth_signTypedData_v4 for EIP-712 challenges, secp256k1_sign for the Hedera transaction digest). The gap we hit: for a raw 32-byte digest, a wallet policy has no amount, recipient or method to evaluate, so on a chain whose native transfers are not EVM transactions (Hedera) our per-session spend limit had to live in the gateway rather than in a Privy policy. We did end up using the other half of what we originally asked for - an authorization-key requirement scoped per delegated wallet: every signSecp256k1/signTypedData call for a principal's own wallet now has to carry an authorization_context signed with a P-256 key registered on that wallet's signer quorum, so raw-hash signing is not just "the server wallet signs whatever we ask" - a request missing that authorization context is not a well-formed call in the first place. What is still missing is a native signing *budget* on raw-hash signing itself (a per-wallet or per-quorum-member spend cap enforced by Privy, not just an authorization check) - the amount limit still has to live in our gateway because the digest alone carries no amount for a policy to inspect. <add findings from a live test confirming Privy actually rejects a raw-hash signing request that omits or misauthorizes authorization_context, once the OAuth cutover is live>
```
