...
- 決済資産は **ネイティブ HBAR**（Blocky402 の対応制約、v1.8 / 憲章 v1.3.0）。Manifest / JSON-RPCはweibar（10^18 = 1 HBAR）、Solidity境界とオンチェーン会計はtinybar（10^8 = 1 HBAR）。
...
- [x] T007 [P] `.env.example`：`HEDERA_RPC_URL` / `HEDERA_OPERATOR_KEY` / `X402_FACILITATOR_URL`（`https://api.testnet.blocky402.com`）/ `PAYMENT_ASSET`（`native`）/ `ANTHROPIC_API_KEY` / `HYPERDRIVE_URL` / `WEB3_STORAGE_TOKEN` / `SUBGRAPH_URL` / `GRAPH_NODE_HOST` / `GATEWAY_URL` / `AWS_REGION`。**USDC 系 env は無し**
...
- [x] T018 [P] `apps/contracts/scripts/probe-hedera.ts`：最小の `payable` コントラクトを Testnet へ。**ネイティブ HBAR value の挙動**を実測（R-4）：JSON-RPCのtransaction `value` はweibar、Solidityの `msg.value` / `.call{value:}` はtinybar。tinybar単位の `mulDiv` 2者分配、払い出し、返金を確認（2026-09-07）。
...
- [x] T024 `packages/shared/src/eip712.ts`：Domain（`TrueCollective` / `1` / `296` / `verifyingContract`）、`RightsReceipt`（17 フィールド、`price` はtinybar）/ `KeyGateChallenge` / `OwnerAuthChallenge` の types、`computeReceiptHash()`（EIP-712 `hashStruct`）、`computePurchaseRequestHash()`（T023 依存）
...
- [x] T032 [P] `apps/contracts/test/RightsRegistry.revenue.spec.ts`：tinybar単位で `mulDiv` の端数が出る価格を使い、`creatorAmount + ownerAmount == price`（端数はcreator、dustゼロ）、`allocationOf` は再移転後も不変（FR-010）、`claim()` が確定額のみHBARで払い出し・所有者再解決なし・`nonReentrant`（SC-006 / FR-009）
...
- [x] T043 [P] `apps/contracts/contracts/libraries/RevenueLib.sol`：**tinybar 単位**（Hedera Solidityの `msg.value` と同じ単位）の `mulDiv` ベース2者按分と端数規則（**余りは `creator` に固定**。`treasury` は導入しない。SC-006）
...
- [x] T051 `apps/subgraph/src/mappings/rightsRegistry.ts`（`handleReceiptIssued` / `handleReceiptConsumed` / `handleRevenueAllocated`〔金額はtinybar〕/ `handleClaimed` / `handleLicenseEpochBumped`）。`subgraph.yaml` に `RightsRegistry` データソース追加
...
- [x] T115 `pnpm --filter web deploy`（`wrangler pages deploy dist`）。`GATEWAY_URL` / `SUBGRAPH_URL` を本番設定（2026-09-07: `https://truecollective.pages.dev`）
