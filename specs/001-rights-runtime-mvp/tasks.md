...
- [ ] T043 [P] `apps/contracts/contracts/libraries/RevenueLib.sol`：**tinybar 単位**（2026-09-05 R-4 改訂、`msg.value / 1e10` 変換後の値を渡す）の `mulDiv` ベース 2 者按分と端数規則（**余りは `creator` に固定**。`treasury` は導入しない ― 2026-09-06 訂正：T045 の tinybar 会計・creator-only dust 方針と矛盾していたため統一、Codex bounded exec レビュー指摘。SC-006）
