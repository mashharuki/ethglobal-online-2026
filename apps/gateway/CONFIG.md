...
- `DATABASE_URL_PG=<postgres url> pnpm --filter gateway test:pg` (tasks.md Phase 11) runs
  ...
  sets `REQUIRE_PG=1`, which turns a missing/broken connection into a failure instead. That job's
  Postgres service container password comes from the repository variable
  `CI_POSTGRES_PASSWORD` (not a secret - a throwaway value for an ephemeral container discarded
  with the job, kept out of the workflow file only so a scanner has no literal to flag). A fork
  or independent copy of this repo must set that variable (any value) or the service container
  fails to start - GitHub Actions resolves an unset `vars.*` reference to an empty string.
...
| `DATABASE_URL=<direct Postgres> GATEWAY_URL=<deployed> pnpm --filter gateway bootstrap:ci-oauth-client` | tasks.md Phase 10, one-time human step: registers the pre-registered CI `oauth_client` (or reuses it) and runs the real authorize+PKCE+consent flow to mint the non-rotating refresh token `apps/agent`'s CI harness uses. Prints `MCP_CI_CLIENT_ID` (a repository *variable*) and `MCP_CI_REFRESH_TOKEN` (a repository *secret*) for `.github/workflows/ci.yml`'s `agent-autonomous` job - see `apps/agent/README.md`. |
