...
/// @notice Records how native HBAR value behaves on Hedera EVM: msg.value in tinybar,
///         mulDiv dust in tinybar accounting, and `.call{value:}` payouts.
...
contract ProbeSettle {
    event Received(address indexed from, uint256 msgValueTinybar, uint256 priceTinybar);
    event Split(uint256 priceTinybar, uint256 creatorTinybar, uint256 ownerTinybar, uint256 dustTinybar);
    event PaidFor(bytes32 indexed paymentId, address indexed payer, uint256 msgValueTinybar);
    event PaidOut(address indexed to, uint256 amountTinybar, bool ok);
        ...
        payable
    {
        ...
        uint256 got = msg.value;
        emit Received(msg.sender, msg.value, got);
    ...
    function payFor(bytes32 paymentId) external payable {
        ...
        balanceTinybar[msg.sender] += msg.value;
    }
    ...
    function withdraw() external returns (bool ok) {
        ...
        emit PaidOut(msg.sender, amount, ok);
